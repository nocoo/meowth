...
- **CI matrix**:[`.github/workflows/ci.yml`](../../.github/workflows/ci.yml) 在 GitHub macOS runner 上跑 lint / build-darwin(arm64+amd64) / l1-daemon / l1-dashboard / l2 / l2-embed / l3(embed+embed-mint) / g2 / secret-scan

**仍未接入自动化**:D1 隔离扫描、Playwright `dashboard-dev` project(GitHub runner 上 `meowthd init` 易 flake,详 `docs/features/01` §6.2)。这两项需要 release 前手动 `pnpm scan:d1` 与本机跑 dashboard-dev e2e。

...
// list; for providers with a CLI discovery mechanism (pi) it shells
// out with caching and falls back to the static list on failure.
//
// For claude and codex, the catalog is augmented with per-model
...
// `session/set_model` RPC before each prompt; Claude / Codex / Copilot / Pi
// pass it via flag or session config.
//

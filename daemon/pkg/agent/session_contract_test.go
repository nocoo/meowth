...
// TestMessagePipeCloseDrainsAll pins that Close delivers every
// queued message to a late consumer (success-path durability).
func TestMessagePipeCloseDrainsAll(t *testing.T) {
	t.Parallel()
	pipe := newMessagePipe()
	const n = 200
	for i := 0; i < n; i++ {
	...
	if got != n {
		t.Fatalf("Close drain lost messages: got %d want %d", got, n)
	}
}

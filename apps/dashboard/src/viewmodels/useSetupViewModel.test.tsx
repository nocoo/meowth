...
describe('useSetupViewModel — initial state and mode switching', () => {
  it('starts in token mode with an idle status and dev-disabled mint by default', () => {
    const { result } = renderHook(() => useSetupViewModel({ isDev: true }), {
      wrapper: wrap(),
    });
    ...
    expect(result.current.mintDisabledReason).toContain('production build');
  });

  it('enables mint when isDev = false (production embed)', () => {
    const { result } = renderHook(() => useSetupViewModel({ isDev: false }), {
      wrapper: wrap(),
    });
...
describe('useSetupViewModel.submitMint', () => {
  it('refuses to fire fetch when mint is dev-disabled', async () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch');
    const { result } = renderHook(() => useSetupViewModel({ isDev: true }), {
      wrapper: wrap(),
    });
    ...
    expect(window.localStorage.getItem(TOKEN_KEY)).toBeNull();
    if (result.current.status.kind === 'error') {
      expect(result.current.status.message).toContain('production build');
    } else {
      throw new Error(`expected error status, got ${result.current.status.kind}`);
    }
  });
